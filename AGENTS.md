...
- **Bead origin metadata** — `/do:work` automatically adds `"origin": "work"` and `"plan": "<plan-file>"` to `--metadata` when creating beads, making work-created beads structurally distinguishable from manually-created beads. Users creating beads manually do not need to add `origin` — its absence signals manual origin. This enables analytics (estimation accuracy, velocity, cost-per-bead) to separate the two populations without relying on description-prefix conventions.

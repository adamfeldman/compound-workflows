...
- **JSONL format:** Empirically discovered by inspecting `~/.claude/projects/-<path-with-dashes>/*.jsonl`. Entry types: progress, assistant, user, file-history-snapshot, queue-operation, system, custom-title, last-prompt. System subtypes: turn_duration, compact_boundary, local_command.

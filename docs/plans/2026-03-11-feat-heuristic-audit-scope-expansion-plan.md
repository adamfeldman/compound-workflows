...
### init-values.sh candidates (25 patterns)
...
### Command restructure / split-call (15 patterns)
...
| 31 | work.md | L88 | `current_branch=$(git branch ...)` | non-exempt | `git branch --show-current` — no $() |
| 32 | work.md | L89 | `default_branch=$(git symbolic-ref ...)` | non-exempt | same |
| 33 | work.md | L91 | `default_branch=$(git rev-parse ...)` | non-exempt | fallback call |
...
| Technique | Patterns | % |
|-----------|----------|---|
| init-values.sh | 26 | 51% |
| Split-call (command restructure) | 15 | 29% |
...
- `<stem>`: optional topic/plan stem for STATS_FILE construction. Required for commands with stats capture.
...
Example for `bash init-values.sh work my-feature`:
...
| Command | Values |
|---------|--------|
| brainstorm, plan, deepen-plan, review, work | PLUGIN_ROOT, RUN_ID, DATE, STATS_FILE |
| work (additional) | WORKTREE_MGR |
...
2. If local path doesn't exist (installed plugin): `find ~/.claude/plugins -name "init-values.sh" -path "*/compound-workflows/*" | head -1` — also no `$()` in tool input.
...
1. Try relative path from skill file: `../../scripts/init-values.sh` (works in both source repo and installed plugin, since skills are at `skills/<name>/SKILL.md` and scripts are at `scripts/init-values.sh` — both under the plugin root)
2. Fallback: `find ~/.claude/plugins -name "init-values.sh" -path "*/compound-workflows/*" | head -1`

Skill instructions should say: "Run `bash ../../scripts/init-values.sh <skill-name>` from the skill directory. If the script is not found, run the find fallback."

Alternatively, if `$PLUGIN_ROOT` is already known from a prior step in the skill flow, use `$PLUGIN_ROOT/scripts/init-values.sh` directly. The `plugin-changes-qa` and `classify-stats` skills already discover PLUGIN_ROOT — but that discovery itself uses `$()`, which is what init-values.sh replaces. So the bootstrap must use the relative path or find fallback.
...
- PLUGIN_ROOT: try local path, find fallback internally
- REPO_ROOT: `$(git rev-parse --show-toplevel 2>/dev/null || pwd)`
- RUN_ID: `$(uuidgen | cut -c1-8)`
- DATE / DATE_COMPACT / TIMESTAMP: `$(date ...)` variants
...
- [ ] **Backtick detection:** Add secondary regex targeting backtick *substitution* in assignment context: `` [A-Za-z_]+=`[^`]+` `` (variable assignment using backtick substitution). This avoids false positives from Markdown inline code formatting. Scope to lines inside ` ```bash ` code fences where possible. This is a best-effort secondary check — the primary `\$\(` regex catches the vast majority of patterns. Any edge cases may need `# heuristic-exempt`.
...
- [ ] Rewrite L88-91 branch detection as split-call. Replacement instruction prose: "Run `git branch --show-current` and read the output as the current branch name. If the output is empty (detached HEAD), run `git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null | sed 's@^refs/remotes/origin/@@'` and read the output as the default branch. If that also fails, run `git rev-parse --verify origin/main >/dev/null 2>&1 && echo main || echo master` and read the output. Use the branch name to derive the work stem for the init-values.sh call." (No `$()` in any tool input — first token is `git`, no assignment wrapper.)
...
- [ ] Rewrite L412 git commit heredoc: use Write tool to write commit message to a temp file, then `git commit -F <file>` (no `$()` in tool input — `git` first token, no heredoc substitution)
- [ ] Rewrite L427 gh pr create heredoc: use Write tool to write PR body to a temp file, then `gh pr create --body-file <file>` (same pattern)
- [ ] Remove 4 `# heuristic-exempt` markers
...
- **Steps 1 and 2 can run in parallel** — different files, no dependencies
...
1. Agent A: Steps 1 + 2 (foundation — scripts + QA expansion)
2. Agent B: Steps 3a + 3b + 3c (brainstorm, plan, review — simpler commands)
3. Agent C: Steps 3d + 3e + 3f + 3g (work, deepen-plan, compact-prep, setup — complex commands)
4. Agent D: Steps 4a-4g (all skills — smaller, uniform changes)
5. Agent E: Step 5 (verification — depends on B, C, D)

Steps 2-4 can start after Step 1 completes. Step 5 after all others.
...
**Skill bootstrap (from specflow G4)** — Resolved by adding a "Skill Bootstrap" subsection to the init-values.sh Interface Contract. Skills use relative path `../../scripts/init-values.sh` (works in both source repo and installed plugin since the directory structure is consistent) with find fallback.

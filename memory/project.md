...
- **Git index isolation (bead s7qj)** — CLOSED. v3.2.0 shipped but non-functional. **v2 plan (bead wxco)** complete at `docs/plans/2026-03-15-fix-worktree-session-isolation-v2-plan.md`. 9 steps, MINOR bump to v3.3.0. Key additions from red team: git pre-commit hook (deterministic enforcement), PID-based concurrent session detection, integration test checkpoint, hook self-version check. Inherited Assumptions section (ytlk framework) documents 7 load-bearing assumptions. Next: `/do:work`.
- **New beads this session:** dsql (plan should offer supersede, P3), gu2z (gitignored files missing in worktrees, P1), 5pa5 (plans should inline canonical text, P2)

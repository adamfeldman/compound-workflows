...
- **`claude plugin update` unreliable** — command exits 0 silently but doesn't always pull latest. Fallback (plugin dev only): `git -C ~/.claude/plugins/marketplaces/<name> pull origin main` then `/reload-plugins`. Do NOT rsync or manually copy to the cache directory — `claude plugin update` or the git pull fallback handles cache sync. The cache at `~/.claude/plugins/cache/` is managed by Claude Code, not by the developer.

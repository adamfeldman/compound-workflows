...
## Overview
- Plugin: compound-workflows v3.2.0 (plugins/compound-workflows/)
- 26 agents, 30 skills, 8 commands (thin aliases)
...
- **Cache tokens dominate Claude Code cost** — daily: 140M cache read vs 200k I/O tokens. Cache:I/O ratio ~710x. Stats `tokens` field only captures I/O, not cache — per-agent cost requires the cache-inclusive rate. See `memory/cost-analysis.md`.
- **Opus 4.6 pricing is 3x cheaper than Opus 4** — verified 2026-03-14 from Anthropic pricing page. Opus 4.6: $5/$6.25/$0.50/$25 (input/cache_write/cache_read/output per MTok). Opus 4: $15/$18.75/$1.50/$75. The script's `MODEL_PRICING` had Opus 4 rates applied to Opus 4.6 sessions — 3.75x overcharge on cache reads. Phase 6 P0a fixes this. The $4,980 total cost from phase 5 is wrong. Opus-to-Sonnet cache read ratio: 1.67x for Opus 4.6 (was assumed 6x).
...
- **Session worktree start flow (bead hb4a)** — P0 240m. **Deepened (run 1, complete)** at `docs/plans/2026-03-16-feat-session-worktree-start-flow-plan.md`. Full pipeline: 19 agents + synthesis + 3 specflow passes (25+8+5 findings) + 3-provider red team + readiness (2C/8S/9M → all resolved) + convergence ("ready for work") + 5 scenario walkthroughs (2 critical gaps found, fixed) + brainstorm cross-check (10/11 carried). Key additions: Step -1 `.workflows/` root resolution, stale GC lock recovery, session-gc.sh CLI interface, untracked file check, MANDATORY /do:start, PID mismatch escape hatch, rename guard, implementation spike gate. **Incident:** .workflows/ artifacts from session-dp4a nearly lost during worktree merge (recovered from stale worktree) — exactly the bug Step -1 fixes. Next: `/do:work`.
- **Prior session beads:** bo91 (MINOR confidence presentation, P2 45m), rfha (defer-by-default behavior, P2 60m), k4eb (brainstorm assumption traceability, P2 45m)
- **Prior session beads:** r0pw (plan MINOR triage skipped — no phase-completion gate, P2 45m)
- **Prior session beads:** 49yg (brainstorm provenance tags P0), 8wlb (phase-level decision resolution P0), zu8u (statusline color bug P2)
- **Empirical findings (2026-03-16):** (1) `$PPID` in SessionStart hook = Claude Code process PID (not tmux/bash). (2) `git status/log/branch` do NOT update directory mtime on macOS/APFS — mtime is safe for freshness heuristic. (3) `git worktree move` leaves `.git/worktrees/` internal metadata with old name — use `git branch -m` + remove/recreate instead.
- **Prior session beads:** dsql (plan should offer supersede, P3), gu2z (gitignored files missing in worktrees, P1), 5pa5 (plans should inline canonical text, P2)

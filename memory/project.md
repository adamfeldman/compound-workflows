...
- **Git index isolation (bead s7qj)** — CLOSED. v3.2.0 shipped (9 commits, 843 lines, 16 files). BUT empirical testing revealed: (1) SessionStart stderr+exit 2 doesn't deliver output, (2) EnterWorktree auto-cleans worktrees on exit. **v2 brainstorm (bead wxco)** replaces EnterWorktree with `bd worktree create`, changes hook to stdout+exit 0. Empirically validated: model auto-creates worktree, persists after exit, resumes into existing worktree. Next: `/do:plan` for v2 changes. See `docs/brainstorms/2026-03-14-session-worktree-v2-brainstorm.md`.

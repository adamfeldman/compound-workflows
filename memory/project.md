...
- **Session worktree start flow (bead hb4a)** — P0 240m. Brainstorm **ready-for-plan** at `docs/brainstorms/2026-03-15-session-worktree-start-flow-brainstorm.md`. 11 Decisions, 23 Resolved Questions (provenance-tagged), 12 Assumptions (3 empirically verified this session). 3 rounds red team (Gemini, OpenAI, Opus), 3 rounds specflow, zero blockers. Key decisions: combined PID+state cleanup (D9), deterministic pre-commit with opt-out sentinel (D10), git plumbing worktree detection (D11), per-claimant PID files, /do:start internal flow, /do:work Phase 1.2 transition. Cross-session race reproduced and root-caused via JSONL forensics. Next: `/do:plan`.
- **New beads this session:** 49yg (brainstorm provenance tags P0), 8wlb (phase-level decision resolution P0), zu8u (statusline color bug P2)

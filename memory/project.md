...
- **Native agent discovery (bead wgl)** — P1, plan complete + red-teamed at `docs/plans/2026-03-09-feat-native-agent-discovery-plan.md`. Replace deepen-plan Phase 2 filesystem discovery + skills discovery with subagent_type registry. 5 steps (exec order: 3→1→2→3b→4): prefix validation, Phase 2 single-pass rewrite (agent + skills discovery + deterministic post-discovery validation + manifest schema + backward compat), Task→Agent dispatch migration (all phases), QA script updates for Agent syntax, version bump + QA. Red team resolved: 3 CRITICAL (deterministic validation for hallucinated agents, QA script blindness, trust gating dismissed), 7 SERIOUS (model inherit filtering, skills find, dedup policy, general-purpose validated, deterministic cap, backward compat descriptions). Post-red-team readiness re-check: 9 edit-induced findings fixed. Next: `/compound:work`.

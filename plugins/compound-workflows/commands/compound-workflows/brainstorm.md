---
name: compound-workflows:brainstorm
...
Brainstorming answers **WHAT** to build through collaborative dialogue. It precedes `/compound-workflows:plan` which answers **HOW**.
...
- Suggest: "Your requirements seem detailed enough for planning. Run `/compound-workflows:plan` instead?"
...
After capturing the design, challenge it with a different AI model. Different training data produces genuinely different blind spots — this catches assumptions Claude wouldn't question.

**Ask the user:**
"Run a red team challenge on this brainstorm? A different model will try to poke holes in the reasoning. (~2 min)"

**If the user declines**, skip to Phase 4.

#### Step 1: Red Team via PAL

Use PAL `chat` with a non-Claude model to get a divergent perspective:
...
=== OUTPUT INSTRUCTIONS (MANDATORY) ===
Write findings to: .workflows/brainstorm-research/<topic-stem>/red-team-critique.md
Return ONLY a 2-3 sentence summary.
"
```

Write PAL's response to: `.workflows/brainstorm-research/<topic-stem>/red-team-critique.md`

#### Step 2: Surface Unresolved Items

Read the red team critique. For each CRITICAL or SERIOUS item, present to the user via **AskUserQuestion**:

"[Red team challenge summary]. How should we handle this?"
...
2. **Proceed to planning** — `/compound-workflows:plan`
3. **Ask more questions** — Continue exploring before moving on
4. **Compound this brainstorm** — If the brainstorm surfaced surprising findings, novel frameworks, or reusable research, run `/compound-workflows:compound` to capture it

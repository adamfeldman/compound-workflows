---
name: compound-workflows:work-agents
description: Execute work plans using subagents per step — context stays lean, progress survives compaction
argument-hint: "[plan file, specification, or todo file path]"
---

# Work Plan Execution — Subagent Architecture

Execute a work plan by dispatching each step to an independent subagent. The main context acts as an **orchestrator only** — it never reads source files or writes code directly. This prevents context exhaustion during long plans.

## How This Differs from `/compound-workflows:work`

| | `/compound-workflows:work` | `/compound-workflows:work-agents` |
|---|---|---|
| **Execution** | Main context does all coding | Subagents do all coding |
| **Context usage** | Fills up over multi-step plans | Stays lean (summaries only) |
| **Compaction risk** | High — loses in-flight state | Low — bd holds all state |
| **Token cost** | Lower per step (shared context) | Higher per step (subagent overhead) |
| **Best for** | Small plans (3-5 steps) | Large plans (5+ steps) |
...
3. **Optional: Dispatch reviewer subagent** for complex changes:
   ```
   mkdir -p .workflows/work-agents-review/
   ...
   Write your COMPLETE findings to: .workflows/work-agents-review/code-simplicity.md

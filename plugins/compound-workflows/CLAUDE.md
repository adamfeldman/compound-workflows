...
10. **Consolidate multi-command setup into scripts, not code blocks** — when a skill needs multiple shell commands for initialization (mkdir, script calls, env checks), fold them into a single script rather than listing them as separate lines in a code block. Models interpret multi-line code blocks as a single Bash call and chain them with `&&`, which triggers compound-command heuristics even when each individual command has a static allow rule. One script call = one first-token match = no prompt. (See: brainstorm/plan/work skills chaining `mkdir && bash init-values.sh`.)

...
<examples>
<example>
Context: User is about to implement a feature involving payment processing.
user: "I need to add payment threading to the auth service"
assistant: "I'll use the learnings-researcher agent to check docs/solutions/ for any relevant learnings about payment processing or auth service implementations."
...
user: "Auth token generation is slow, taking over 5 seconds"
assistant: "Let me use the learnings-researcher agent to search for documented performance issues, especially any involving authentication or N+1 queries."
...
- **Module names**: e.g., "AuthService", "PaymentProcessor", "payments"
...
Grep: pattern="title:.*payment" path=docs/solutions/ output_mode=files_with_matches -i=true
Grep: pattern="tags:.*(payment|billing|stripe)" path=docs/solutions/ output_mode=files_with_matches -i=true
Grep: pattern="module:.*(Auth|Payment)" path=docs/solutions/ output_mode=files_with_matches -i=true
...
```bash
Grep: pattern="payment" path=docs/solutions/ output_mode=files_with_matches -i=true
```
...
- payment_processing, auth_service, assistant, authentication
...
- `/compound:plan` - To inform planning with institutional knowledge
- `/compound:deepen-plan` - To add depth with relevant learnings

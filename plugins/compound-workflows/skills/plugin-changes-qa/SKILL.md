...
Run `bash ${CLAUDE_SKILL_DIR}/../../scripts/init-values.sh plugin-changes-qa`. Read the output and track REPO_ROOT, PLUGIN_ROOT, MAIN_ROOT, WORKFLOWS_ROOT, DATE, and RUN_ID values.
...
```bash
mkdir -p $WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/agents/
```
...
Write your COMPLETE findings to: $WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/agents/context-lean-review.md
...
=== OUTPUT INSTRUCTIONS (MANDATORY) ===
Write your COMPLETE findings to: $WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/agents/role-description-review.md
...
=== OUTPUT INSTRUCTIONS (MANDATORY) ===
Write your COMPLETE findings to: $WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/agents/completeness-review.md
Structure with: ## Summary, ## Findings (grouped by file), ## Recommendations
After writing the file, return ONLY a 2-3 sentence summary.
DO NOT return your full analysis in your response.
"
...
```bash
ls $WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/agents/
```
...
```bash
ls $WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/agents/*.md
```
...
```bash
mkdir -p $WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/
...
Write the JSON output to `$WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/open-beads.json` (single-fetch pattern — fetched once here, referenced by all subsequent steps in Phase 3.3).
...
- Beads JSON from `$WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/open-beads.json` (written in Step 3.3.1)
...
**Beads data:** Read the open beads JSON from `$WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/open-beads.json`. This file contains all open beads with their IDs, titles, descriptions, and notes.
...
=== OUTPUT INSTRUCTIONS (MANDATORY) ===
Write your COMPLETE classification to: $WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/bead-cross-ref-matches.md
...
```bash
ls $WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/bead-cross-ref-matches.md 2>/dev/null && echo "EXISTS" || echo "NOT_FOUND"
```
...
When the file exists, read results from `$WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/bead-cross-ref-matches.md` using the Read tool. Merge any `matched` results with the deterministic matches from Step 3.3.2 to build the combined matched-findings list for the next step.
...
**Beads data:** Read the full bead details from `$WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/open-beads.json` to access each bead's description and notes.
...
=== OUTPUT INSTRUCTIONS (MANDATORY) ===
Write your COMPLETE assessment to: $WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/bead-cross-ref-coverage.md
...
```bash
ls $WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/bead-cross-ref-coverage.md 2>/dev/null && echo "EXISTS" || echo "NOT_FOUND"
```
...
When the file exists, read results from `$WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/bead-cross-ref-coverage.md` using the Read tool.
...
- `$WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/bead-cross-ref-matches.md` (LLM matching results from Step 3.3.3, if it ran)
- `$WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/bead-cross-ref-coverage.md` (coverage assessment from Step 3.3.4, if it ran)
...
Write the composed tracking status presentation to `$WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/bead-cross-ref-batch.md`. This file serves two purposes: (1) recovery if context compacts during the confirmation flow, and (2) keeping the full presentation data on disk rather than solely in context.
...
- Agent outputs go to `$WORKFLOWS_ROOT/plugin-qa/<DATE>-<RUN_ID>/agents/`. Each run writes to its own dated subdirectory; old run results are retained.

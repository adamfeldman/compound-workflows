...
**Organization:** Single-file architecture - each problem documented as one markdown file in its symptom category directory (e.g., `docs/solutions/performance-issues/n-plus-one-auth-service.md`). Files use YAML frontmatter for metadata and searchability.
...
- `missing-include-AuthService-20251110.md`
- `parameter-not-saving-state-PaymentProcessor-20251110.md`
...
Action:
1. Prompt: "What should the new skill be called? (e.g., stripe-billing, payment-processing)"
...
2. **Gather context:**
   - Module: Auth Service
   - Symptom: Auth token generation taking >5 seconds, N+1 query when loading user sessions
   - Failed attempts: Added pagination (didn't help), checked background job performance
   - Solution: Added eager loading with `includes(:sessions)` on AuthService model
   - Root cause: Missing eager loading causing separate database query per user session
3. **Check existing:** No similar issue found
4. **Generate filename:** `n-plus-one-auth-token-generation-AuthService-20251110.md`
5. **Validate YAML:**
   ```yaml
   module: Auth Service
   date: 2025-11-10
   ...
   symptoms:
     - "N+1 query when loading user sessions"
     - "Auth token generation taking >5 seconds"
...
6. **Create documentation:**
   - `docs/solutions/performance-issues/n-plus-one-auth-token-generation-AuthService-20251110.md`
